...
<body>
	...
	<div class="utama">
		...
		<table cellspacing="1" align="center" border="1">
			...
			<?php 
			$data = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM qw_absen"));
			...
			//if($bulan == $ambil_bulan){
				$sql = mysqli_query($koneksi, "SELECT SUM(hadir) AS hadir, SUM(sakit) AS sakit, SUM(ijin) AS ijin, SUM(alpa) AS alpa, nis, nama, rombel, tgl_absen, id_rombel FROM qw_absen WHERE id_rombel='$_GET[rombel]' GROUP BY nis");
				$no = 0;
